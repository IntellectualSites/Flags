...
public abstract class NumberFlag<N extends Number & Comparable<N>, F extends AbstractFlag<N, F>>
    extends AbstractFlag<N, F> {
    protected final N minimum;
    protected final N maximum;

    protected NumberFlag(@NotNull N value, N minimum, N maximum) {
        super(value);
    ...
    @Override public F parse(@NotNull String input) throws FlagParseException {
        ...
        if (parsed.compareTo(minimum) < 0 || parsed.compareTo(maximum) > 0) {
            throw new FlagParseException(this, input, String.format("Number has to be in the interval [" + minimum + ", " + maximum + "]"));
        }
